...
public class client {
    ...
    private void configureSocket() {
        System.out.println("[Client] Bitte geben sie den Port an, von dem sie sich zum Server verbinden wollen:");
        try {
            int port = Integer.parseInt(scanner.nextLine());
            socket.bind(new InetSocketAddress(Inet4Address.getLocalHost(), port));
        } catch (IOException e) {
            System.out.println("[Client] Error, try using a different Port");
            e.printStackTrace();
            try {
                Thread.sleep(2000);
    ...
    private void connectToServer() {
        ...
        System.out.println("[Client] Bitte gebe den Server Port an:");
        int port = scanner.nextInt();
        this.serverAddress = new InetSocketAddress(hostname, port);
        System.out.println("[Client] Die Server Address ist: " + this.serverAddress);

        try {
            try { //Tries to connect to the Server
    ...
    private void placeShip(int size, int x, int y, String direction) {
    ...
    private void checkHit(int a, int b) {
        if (shipField[a][b] > 0) {
            sentBool(true);
    ...
    //Send to the Server
    private void sentInt(int integer) {
        try {
    ...
    private void sentUTF(String str) {
        try {
    ...
    private void sentBool(boolean b) {
        try {
